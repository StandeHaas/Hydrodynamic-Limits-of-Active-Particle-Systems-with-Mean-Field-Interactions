...
def D_eff_global(beta, m_beta, gamma_eff, lambda_eff):
    return gamma_eff + (lambda_eff**2) / 2 / (np.cosh(beta * m_beta)**3)
...
def D_eff_theory(beta, m_beta, gamma_eff, lambda_eff, m_beta_non, rho_bar, K):
    return gamma_eff +  lambda_eff**2  / 2 * (1 - rho_bar / K) / (np.cosh(beta * m_beta_non)**3) 
...
def D_eff_theory_4(beta, m_beta, gamma_eff, lambda_eff, m_beta_non, rho_bar, K):
    return gamma_eff + lambda_eff**2 / 2 * (1 - rho_bar / K) / np.cosh(beta * m_beta_non) * (abs(1 - rho_bar / K - rho_bar / K * m_beta_non**2))
...
def sweep_over_betas(beta_values, n_runs_per_beta=10, run=True, save_dict=None, sigma=0.005):
    run_kwargs = dict(
            T = 20,
            obs_dt = 0.1,
            record_fft=True,
    ...
    if run:
        ps_kwargs = dict(
            L = 1000,
            xlim = 1,
            rate_diffusion = 0.002,
            rate_active = 5,
            flip_rate_fn = None,
            init = 'poisson',
            N = 500,
            scale_rates = False,
            local_kernel_sigma = sigma, #0.2, #0.005, # 0.02, #0.005,
            minus_anchor = True,
...
def sweep_over_sigmas(
    ...
    if run:
        ...
        for sigma in sigma_values:
            ...
            results[sigma] = {
                "beta": beta_values,
                "v_mean": save_dict["means"],
                "v_se": save_dict["ses"],
                "D_mean": save_dict["D_means"],
                "D_se": save_dict["D_ses"],
                "ps_kwargs": save_dict["ps_kwargs"],
            }

            np.savez(
                f"v_eff_vs_beta_sigma_{sigma:.4g}.npz",
                beta=beta_values,
                v_mean=save_dict["means"],
                v_se=save_dict["ses"],
                D_mean=save_dict["D_means"],
            ...
            print("mean v_eff =", save_dict["means"])
            print("SE v_eff   =", save_dict["ses"])
    else: 
        data = np.load("v_eff_all_sigmas.npz", allow_pickle=True)
        results = data["results"].item() 
    return results

def plot_v_eff_all_sigmas(results):
    plt.figure(figsize=(7, 5))
    blues = cm.Blues(np.linspace(0.35, 0.9, len(results)))

    # Use ps_kwargs from first entry for theory
    first_key = list(results.keys())[0]
    ps_kwargs = results[first_key]["ps_kwargs"]

    K = ps_kwargs["site_capacity"]
    rho_bar = ps_kwargs["N"] / ps_kwargs["L"]
    dx = ps_kwargs["xlim"] / ps_kwargs["L"]
    lambda_eff = ps_kwargs["rate_active"] * dx
    ...
    v_non = v_pred_without_phi(lambda_eff, m_beta_dense, beta_dense)
    v_tasep = v_pred_TASEP(lambda_eff, rho_bar, K, m_beta_dense)
    v_excl = v_eff_fit(rho_bar, K, beta_dense, lambda_eff, m_beta_dense, m_beta_non)

    for (sigma, color) in zip(results.keys(), blues):
        beta = results[sigma]["beta"]
        v_mean = results[sigma]["v_mean"]
        v_se = results[sigma]["v_se"]
    ...
    # Theory (dashed)
    plt.plot(beta_dense, v_non, '--', color='navy', label='non-exclusion')
    #plt.plot(beta_dense, v_excl, '--', color='navy', label='exclusion')
    plt.plot(beta_dense, v_tasep, '--', color='royalblue', label='exclusion')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("v_eff_vs_beta_all_sigmas.png", dpi=200)
    plt.close()

def plot_D_eff_all_sigmas(results):
    plt.figure(figsize=(7, 5))
    blues = cm.Blues(np.linspace(0.35, 0.9, len(results)))

    # Use ps_kwargs from first sigma for theory
    sigmas = sorted(results.keys())
    first_sigma = sigmas[0]
    ps_kwargs = results[first_sigma]["ps_kwargs"]

    K = ps_kwargs["site_capacity"]
    rho_bar = ps_kwargs["N"] / ps_kwargs["L"]
    dx = ps_kwargs["xlim"] / ps_kwargs["L"]
    lambda_eff = ps_kwargs["rate_active"] * dx
    D0 = ps_kwargs["rate_diffusion"] * (dx**2)

    beta_dense = np.linspace(0, 3, 400)

    # --- theory terms ---
    m_beta = compute_m_of_beta(beta_dense, rho_bar, K, lambda_eff)

    D_global = D_eff_global(beta_dense, m_beta, D0, lambda_eff)
    D_theory = D_eff_theory(beta_dense, m_beta, D0, lambda_eff, m_beta, rho_bar, K)
    D_theory_4 = D_eff_theory_4(beta_dense, m_beta, D0, lambda_eff, m_beta, rho_bar, K)

    # --- plot simulation data ---
    for (sigma, color) in zip(sigmas, blues):
        beta = results[sigma]["beta"]
        D_mean = results[sigma]["D_mean"]
        D_se = results[sigma]["D_se"]

        plt.errorbar(
            beta,
            2.5*np.array(D_mean),
            yerr=2.5*np.array(D_se),
            fmt='o',
            capsize=3,
            color=color,
            label=rf"$\sigma = {sigma}$",
    ...
    # --- theory curves (dashed) ---
    plt.plot(beta_dense, D_global * np.ones_like(beta_dense),
             '--', color='navy', label=r'theory: non-exclusion')

    plt.plot(beta_dense, D_theory_4,
             '--', color='royalblue', label=r'theory_exclusion')
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("D_eff_vs_beta_all_sigmas.png", dpi=200)
    plt.close()

def plot_v_eff_vs_sigma_all_beta(results):
    sigmas_raw = sorted(results.keys())
    sigmas_pos = [s for s in sigmas_raw if s > 0]  # strictly positive σ

    beta_vals = results[sigmas_pos[0]]["beta"]

    plt.figure(figsize=(7, 5))
    blues = cm.Blues(np.linspace(0.35, 0.9, len(beta_vals)))

    for i, (beta, color) in enumerate(zip(beta_vals, blues)):
        v_pos = []
        v_err_pos = []

        for s in sigmas_pos:
            v_pos.append(results[s]["v_mean"][i])
            v_err_pos.append(results[s]["v_se"][i])

        # continuous line: σ > 0 only (includes σ = 1)
        plt.errorbar(
            sigmas_pos,
            v_pos,
            yerr=v_err_pos,
        ...
        # σ = 0 plotted separately at σ = 1
        if 0 in results:
            plt.errorbar(
                1.0,
                results[0]["v_mean"][i],
                yerr=results[0]["v_se"][i],
                fmt='o',
                markersize=6,
                capsize=3,
                color=color,
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("v_eff_vs_sigma_all_beta.png", dpi=200)
    plt.close()

def plot_D_eff_vs_sigma_all_beta(results):
    sigmas_raw = sorted(results.keys())
    sigmas_pos = [s for s in sigmas_raw if s > 0]
    ...
    for i, (beta, color) in enumerate(zip(beta_vals, blues)):
        ...
        for s in sigmas_pos:
            D_pos.append(results[s]["D_mean"][i])
            D_err_pos.append(results[s]["D_se"][i])

        plt.errorbar(
            sigmas_pos,
            2.5*np.array(D_pos),
            yerr=2.5*np.array(D_err_pos),
            fmt='o',
            capsize=3,
            color=color,
            label=rf"$\beta = {beta:.2f}$",
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("D_eff_vs_sigma_all_beta.png", dpi=200)
    plt.close()

sigma_values = [1e-4, 3e-4, 1e-3, 3e-3, 1e-2, 3e-2, 0.1, 0.3, 0]
beta_values = np.linspace(0, 3, 11)
run = True

all_results = sweep_over_sigmas(
...
if run:
    np.savez("v_eff_all_sigmas.npz", results=all_results)

plot_v_eff_all_sigmas(all_results)
plot_D_eff_all_sigmas(all_results)
plot_v_eff_vs_sigma_all_beta(all_results)
plot_D_eff_vs_sigma_all_beta(all_results)